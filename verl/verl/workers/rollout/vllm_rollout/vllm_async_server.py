...
class AsyncvLLMServer(AsyncServerBase):
    ...
    async def generate(self, prompt_ids: list[int], sampling_params: dict[str, Any], request_id: str) -> list[int]:
        # Respect per-call max_tokens if provided; otherwise fall back to rollout.response_length.
        # Always cap by the engine's max capacity for this prompt.
        capacity = max(0, self.max_model_len - len(prompt_ids))
        ...
        # Fallback to configured response_length when explicit per-call cap is not provided
        default_resp_len = None
        try:
            default_resp_len = int(getattr(self.config.rollout, "response_length", None))
        except Exception:
            default_resp_len = None

        if requested is None:
            requested = default_resp_len if default_resp_len is not None else capacity
        max_tokens = max(0, min(int(requested), capacity))
