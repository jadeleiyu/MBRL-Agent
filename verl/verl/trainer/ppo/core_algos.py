...
def compute_policy_loss_gspo(
    ...
    """GSPO policy loss with sequence-level importance ratio.

    Reference: https://arxiv.org/pdf/2507.18071

    Args:
        old_log_prob: (bs, T) log-prob under old policy
        log_prob: (bs, T) log-prob under current policy
        advantages: (bs, T) advantage per token (often per-seq broadcast)
        response_mask: (bs, T) mask over response tokens
        loss_agg_mode: aggregation for agg_loss; GSPO recommends seq-mean-token-mean
        config: actor config providing clip ratios (clip_ratio, clip_ratio_low/high)
        rollout_is_weights: optional importance weights per token

    Returns:
        pg_loss, pg_clipfrac, ppo_kl, pg_clipfrac_lower
    """

    # Resolve clip ratios from config if provided
    clip_ratio = getattr(config, "clip_ratio", None)
    ...
    if clip_ratio_low is None:
        clip_ratio_low = clip_ratio if clip_ratio is not None else 0.2
    if clip_ratio_high is None:
        clip_ratio_high = clip_ratio if clip_ratio is not None else 0.2

    # per-token approx KL (current - old)
    negative_approx_kl = log_prob - old_log_prob
    # sequence-average approx KL per example
    seq_lengths = torch.sum(response_mask, dim=-1).clamp(min=1)
    negative_approx_kl_seq = torch.sum(negative_approx_kl * response_mask, dim=-1) / seq_lengths

    # seq-level importance ratio, broadcast to tokens in log-space
    ...
    # Clipped policy gradient with GSPO ratio
    ...
    # Aggregate per GSPO recommendation at sequence level
    pg_loss = agg_loss(loss_mat=pg_losses, loss_mask=response_mask, loss_agg_mode=loss_agg_mode)

    # Metrics for compatibility with PPO-style reporting
    pg_clipfrac = verl_F.masked_mean(torch.gt(pg_losses2, pg_losses1).float(), response_mask)
