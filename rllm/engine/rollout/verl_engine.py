...
class VerlEngine(RolloutEngine):
    ...
    async def get_model_response(self, messages: list[dict], **kwargs) -> ModelOutput:
        ...
        # Determine max_tokens for this call from sampling params if provided,
        # otherwise default to engine's response length. Keep it in sampling_params
        # so that the async server can honor per-call caps, and also retain a local
        # copy to trim client-side if server overshoots.
        max_tokens = sampling_params.get("max_tokens", sampling_params.get("max_new_tokens", self.max_response_length))
        try:
            max_tokens = int(max_tokens)
        except Exception:
            max_tokens = self.max_response_length
        # Ensure the per-call cap is sent downstream to the async server.
