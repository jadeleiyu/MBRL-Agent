...
class WorldModelWebEnv(BaseEnv):
    ...
    async def step_async(self, action: Any) -> tuple[dict, float, bool, dict]:
        """Async version for concurrent trajectory execution"""
        action_str = "" if action is None else str(action)
        has_stop_signal, stop_answer = self._extract_stop_answer(action_str)
        ...
        if has_stop_signal:
            ...
            try:
                logger.debug("World model step action=%s", action_str or "(empty)")
                # Use async version of world model
                wm_cot_list, next_obs_list = await self.world_model.step_async(actions, [self._dreamed_traj], [self.task])
            except Exception as exc:  # noqa: BLE001
                logger.exception("World model step failed")
                raise exc
        ...
        if has_stop_signal:
            is_task_completed = await self._check_task_completion_async(next_obs_text)
            reward = 1.0 if is_task_completed else 0.0
            done = True
        else:
            is_task_completed = False
    ...
    async def _check_task_completion_async(self, current_obs: str) -> bool:
        """Async version of _check_task_completion"""
        objective = self.task.get("objective", "")
        try:
            return bool(await self.world_model.judge_completion_async(objective=objective, current_obs=current_obs))
        except Exception as exc:  # noqa: BLE001
            logger.warning("World model judgment failed: %s", exc)
            return False
