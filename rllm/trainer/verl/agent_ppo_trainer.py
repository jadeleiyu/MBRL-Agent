...
class AgentPPOTrainer(RayPPOTrainer):
    ...
    def visualize_trajectory(self, tensor_batch, sample_idx=0, max_samples=1, mask_key="response_mask"):
        ...
        attention_mask = tensor_batch.batch["attention_mask"]

        prompt_len = prompts.shape[1]
        resp_len = responses.shape[1]
        prompt_attn_mask = attention_mask[:, :prompt_len]
        response_attn_mask = attention_mask[:, -resp_len:]

        batch_size = prompts.shape[0]
        end_idx = min(sample_idx + max_samples, batch_size)

        save_dir = os.path.join(self.config.trainer.default_local_dir, "trajectory_visualizations")
        os.makedirs(save_dir, exist_ok=True)
        file_path = os.path.join(save_dir, f"step_{self.global_steps}.jsonl")

        with open(file_path, "a", encoding="utf-8") as f:
            for i in range(sample_idx, end_idx):
                prompt_tokens = prompts[i].tolist()
                prompt_valid_mask = prompt_attn_mask[i].bool().tolist()
                prompt_token_ids = [tok for tok, valid in zip(prompt_tokens, prompt_valid_mask) if valid]
                prompt_text = self.tokenizer.decode(prompt_token_ids, skip_special_tokens=False)

                resp_tokens = responses[i].tolist()
                resp_valid_mask = response_attn_mask[i].bool().tolist()
                loss_mask = traj_mask[i].bool().tolist()
                scores = token_level_scores[i].tolist() if token_level_scores is not None else [0.0] * len(resp_tokens)

                response_items = []
                for tok_id, valid, include_loss, score in zip(resp_tokens, resp_valid_mask, loss_mask, scores, strict=False):
                    if not valid:
                        continue
                    ...
                    response_items.append(
                        {
                            "token_id": tok_id,
                            "text": token_text,
                            "include_in_loss": bool(include_loss),
                            "score": float(score),
                        }
                    )
