...
class LlamaChatTemplateParser(ChatTemplateParser):
    ...
    def parse_completion(self, completion_ids):
        ...
        # Strip leading assistant header if the model echoed it back.
        if completion_text.startswith(self.assistant_token):
            completion_text = completion_text[len(self.assistant_token) :]

        # Remove trailing special tokens produced by llama chat template.
        trailing_tokens = [
        ...
        for token in trailing_tokens:
            if token and completion_text.endswith(token):
                completion_text = completion_text[: -len(token)].strip()
