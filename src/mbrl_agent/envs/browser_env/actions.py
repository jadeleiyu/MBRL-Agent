...
# @beartype
# def create_id_based_action_gpt_oss(action_str: str) -> Action:
#     """Main function to return individual id based action"""
#     action_str = action_str.strip()
#     action = json.loads(action_str)

#     if action['action'] == 'click':
#         return create_click_action(element_id=action['arg_0'])
    
#     if action['action'] == 'hover':
#         return create_hover_action(element_id=action['arg_0'])
    
#     if action['action'] == 'type':
#         text, element_id = action['arg_1'], action['arg_0']
#         if action['arg_2'] == 1:
#             text += '\n'
#         return create_type_action(text=text, element_id=element_id)
    
#     if action['action'] == 'press':
#         return create_key_press_action(key_comb=action['arg_0'])

#     if action['action'] == 'scroll':
#         return create_scroll_action(direction=action['arg_0'])
    
#     if action['action'] == 'goto':
#         return create_goto_url_action(url=action['arg_0'])
...
#     if action['action'] == "tab_focus":
#         return create_page_focus_action(int(action['arg_0']))
    
#     if action['action'] == "close_tab":
#         return create_page_close_action()
    
#     if action['action'] == "stop":  # stop answer
#         return create_stop_action(action['arg_0'])
